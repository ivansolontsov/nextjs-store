...
const Details: React.FC<Props> = ({ data }) => {
  ...
  const isAlreadyInCart = cart.some(el => el.product.id === data.id) // ПРОВЕРКА НА НАЛИЧИЕ ТОВАРА В КОРЗИНЕ
  const isAlreadyInFav = favorites.some(el => el.id === data.id) // ПРОВЕРКА НА НАЛИЧИЕ ТОВАРА В FAVORITES
